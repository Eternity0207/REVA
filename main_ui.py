...
class MainApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("REVA - Self Operating Computer")
    ...
    def start_fastapi_server(self):
        def run_fastapi():
            try:
                ...
                print(f"[REVA] Starting FastAPI server...")
                uvicorn.run("operate.main_server:app", host="127.0.0.1", port=8002, log_level="warning")
            except Exception as e:
                print(f"[REVA] FastAPI server error: {str(e)}")
